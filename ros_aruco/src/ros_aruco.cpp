...
// Modified by Tolegen and Zhanat on October 2021
// This version of code last modofoed by Zhanat on May 2022
// This code is needed to receive data of aruco markers and 
// perform localization of the user in the research building
//
#include <ros/ros.h>
...
#define SERVER_PORT_to_receive_aruco_marker_data htons(10105)
#define SERVER_PORT_to_send_user_position_data htons(10112)
...
int ArUcoNode::arucoDetect() {
    ...
    int serverSock_receiving_aruco_marker_data=socket(AF_INET, SOCK_STREAM, 0);
    int serverSock_sending_user_position_data=socket(AF_INET, SOCK_STREAM, 0);

    sockaddr_in serverAddr_receiving_aruco_marker_data;
    serverAddr_receiving_aruco_marker_data.sin_family = AF_INET;
    serverAddr_receiving_aruco_marker_data.sin_port = SERVER_PORT_to_receive_aruco_marker_data;
    serverAddr_receiving_aruco_marker_data.sin_addr.s_addr = INADDR_ANY;
    
    sockaddr_in serverAddr_sending_user_position_data;
    serverAddr_sending_user_position_data.sin_family = AF_INET;
    serverAddr_sending_user_position_data.sin_port = SERVER_PORT_to_send_user_position_data;
    serverAddr_sending_user_position_data.sin_addr.s_addr = INADDR_ANY;

    bind(serverSock_receiving_aruco_marker_data, (struct sockaddr*)&serverAddr_receiving_aruco_marker_data, sizeof(struct sockaddr));
    bind(serverSock_sending_user_position_data, (struct sockaddr*)&serverAddr_sending_user_position_data, sizeof(struct sockaddr));

    // wait for a client
    listen(serverSock_receiving_aruco_marker_data,1);
    listen(serverSock_sending_user_position_data,1);
    
    sockaddr_in clientAddr_receiving_aruco_marker_data;
    sockaddr_in clientAddr_sending_user_position_data;
    socklen_t sin_size=sizeof(struct sockaddr_in);

    int clientSock_receiving_aruco_marker_data=accept(serverSock_receiving_aruco_marker_data,(struct sockaddr*)&clientAddr_receiving_aruco_marker_data, &sin_size);

    int counter=0;
    int n;
    
    int clientSock_sending_user_position_data=accept(serverSock_sending_user_position_data,(struct sockaddr*)&clientAddr_sending_user_position_data, &sin_size);
    ...
    while(ros::ok()){
        ...
        //receive a buffer
        n = read(clientSock_receiving_aruco_marker_data, buffer, 500);
        ...
        send(clientSock_sending_user_position_data, oss.str().c_str(), oss.str().length(), 0);
...
int ArUcoNode::arucoDetectZhanat(){
    ...
    while (ros::ok())
    {
        ...
        pose_pub_user_position_data_for_hololens.publish(msg);
...
ArUcoNode::ArUcoNode():n_h_("") {
    ...
    pose_pub_user_position_data_for_hololens = n_h_.advertise<std_msgs::String>("/chatter", 1000);
